...
import { useGame, createInitialState } from '@/context/GameContext';
...
export default function ResultsScreen() {
  ...
  const winner = result ? state.players[result.winnerIndex] : null;
  const loser  = result?.loserIndex !== undefined ? state.players[result.loserIndex] : null;
  ...
  return (
    <View style={styles.overlay}>
      <LinearGradient colors={bgColors} style={[styles.modal, { paddingTop: topPad + 16, paddingBottom: botPad + 16 }]}>
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
            ...
            <Animated.View style={[styles.winnerCard, { opacity: fadeAnim }]}>
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={styles.winnerSeat}>
                  {WIND_CHARS[winner.seatWind - 1]} Seat · {result.isTsumo ? 'Tsumo' : `Ron vs ${loser?.name ?? '?'}`}
                </Text>
              </View>
              <View style={styles.scoreBox}>
                ...
                <Text style={styles.scoreHan}>{result.score.label}</Text>
              </View>
            </Animated.View>
          )}
              ...
              {result.yaku.map((y, i) => (
                <View key={i} style={styles.yakuRow}>
                  ...
                  <View style={styles.hanPills}>
                    {Array.from({ length: y.han }).map((_, j) => (
          ...
          <View style={styles.buttons}>
            <TouchableOpacity style={styles.playAgainBtn} onPress={handlePlayAgain} activeOpacity={0.85}>
              <LinearGradient colors={[colors.primary, '#A07820']} style={styles.playAgainInner} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }}>
...
const styles = StyleSheet.create({
  ...
  titleWin: {
    ...
    letterSpacing: 4,
    textShadow: `0px 0px 20px ${colors.primary}`,
  },
  titleLoss: {
